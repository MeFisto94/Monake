...
selector( "slider.right.button", "monake" ) {
    text = "+"
    background = doubleGradient.clone()
    background.setColor(color(0.5, 0.75, 0.75, 0.5))
...
selector( "slider.up.button", "monake" ) {
    buttonCommands = sliderButtonCommands;
}

selector( "slider.down.button", "monake" ) {
    buttonCommands = sliderButtonCommands;
}

selector( "checkbox", "monake" ) {
...
selector( "rollup", "monake" ) {
    background = gradient.clone()  
    background.setColor(color(0.25, 0.5, 0.5, 0.5))
}

selector( "tabbedPanel", "monake" ) {
    activationColor = color(0.8, 0.9, 1, 0.85f)
}

selector( "tabbedPanel.container", "monake" ) {
    background = null
}

selector( "tab.button", "monake" ) {
    background = gradient.clone()
    background.setColor(color(0.25, 0.5, 0.5, 0.5))
